...
public class PasswordInputActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    private static final String TAG = "PasswordInputActivityInfo";
    ...
    private void checkPassword() {
        ...
        if (!TextUtils.isEmpty(passwordInput)) {
            ...
            if (appLockPassword == null) {
                ...
                //表明第一次进入此界面，需要初始化密码，并保存密码，然后跳转到AppLockActivity，销毁当前活动
                msharedPreferences.edit()
                        .putString("app_lock_password", passwordInput).commit();
                startActivity(new Intent(PasswordInputActivity.this, AppLockActivity.class));
                finish();

            } else if (passwordInput.equals(appLockPassword)) {

                //表明已经设置过密码，并且输入密码正确，跳转到AppLockActivity，销毁当前活动
                startActivity(new Intent(PasswordInputActivity.this, AppLockActivity.class));
                finish();

            } else {
                //表明已经设置过密码，但是输入密码不正确，给予友好提示
                Toast.makeText(PasswordInputActivity.this, "密码错误，请重新输入",
                        Toast.LENGTH_SHORT).show();
            }

        } else {
            //友好提示
            Toast.makeText(PasswordInputActivity.this, "输入框内容不能为空",
                    Toast.LENGTH_SHORT).show();
        }
    }
}
